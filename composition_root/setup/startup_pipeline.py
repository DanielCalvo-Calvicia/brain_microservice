...
def start_startup_pipeline(service: BrainService) -> asyncio.Task:
    logger.info("starting voice pipeline - streams will stay active until shutdown")
...
async def _run_startup_pipeline(service: BrainService) -> None:
    try:
        ...
        logger.info("voice pipeline cancelled by shutdown")
        raise
    except Exception as exc:
        logger.exception("voice pipeline failed", error_type=type(exc).__name__, error=str(exc))
