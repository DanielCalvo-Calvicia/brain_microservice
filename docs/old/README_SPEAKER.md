...
```bash
curl -N -X POST "http://127.0.0.1:8003/play/stream?sample_rate=24000&channels=1" \
...
- HTTP and autoload audio payloads are streamed as `bytes`; `POST /play/stream` wraps `request.stream()` in the inbound playback DTO.

Caching strategy:

- No persistent cache.
- Runtime buffering uses in-memory `asyncio.Queue` inside the outbound playback worker.
- `POST /play/stream` does not buffer the audio request body before passing it to the service.
...
- Any client that can reach `POST /play/stream` can stream arbitrary bytes to the audio device.
